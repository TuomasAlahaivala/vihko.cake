<?php
	...
	$calendar= '<tr style="display:none;"><td><script type="text/javascript">
	jQuery(document).ready(function () {
	varatutpaivat = '.$varaukset['dates'].';
			...
			if($date > $today){
				...
				if($varaukset){
					...
					if (isset($isinarray)) {
						...
						if($varaukset[$varausid][$str] == 1){
                                                 
							$color = "green";
							$jsfunctio = "";

						}else if($varaukset['tila'][$str] == 0){
						...
						if($varaukset['datespaikka'][$str]){
							$title .= "Paikka: ".$varaukset['datespaikka'][$str];
						}
